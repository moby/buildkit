...
func TestDetectCompressionType(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "uncompressed", dt: []byte("payload"), want: Uncompressed},
		{name: "gzip", dt: []byte{0x1f, 0x8b, 0x08}, want: Gzip},
		{name: "zstd", dt: []byte{0x28, 0xb5, 0x2f, 0xfd}, want: Zstd},
		{name: "zstd skippable frame", dt: []byte{0x50, 0x2a, 0x4d, 0x18, 0x00, 0x00, 0x00, 0x00}, want: Zstd},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()

			r := bytes.NewReader(tc.dt)
			got, err := detectCompressionType(io.NewSectionReader(r, 0, int64(len(tc.dt))))
			require.NoError(t, err)
