...
func NewMinioServer(t *testing.T, sb integration.Sandbox, opts MinioOpts) (address string, bucket string, cl func() error, err error) {
	...
	runMc := func(args ...string) error {
		ctx, cancel := context.WithTimeoutCause(t.Context(), mcTimeout, errors.WithStack(context.DeadlineExceeded))
		defer cancel()
		err := integration.RunCmd(mcCmd(ctx, args...), sb.Logs())
		if err != nil && context.Cause(ctx) != nil && context.Cause(t.Context()) == nil {
