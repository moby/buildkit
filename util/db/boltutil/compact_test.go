...
func TestCompactRechecksReclaimabilityAfterDrain(t *testing.T) {
	d := openTestDB(t, filepath.Join(t.TempDir(), "test.db"))
	fillTestDB(t, d, 1000)
	deleteTestKeys(t, d, 0, 900)
	stats, err := d.CompactionStats()
	require.NoError(t, err)
	require.Greater(t, stats.Reclaimable, int64(1<<20))
	started := make(chan struct{})
	release := make(chan struct{})
	writer := make(chan error, 1)
	go func() {
		writer <- d.Update(func(tx *bolt.Tx) error {
			close(started)
			<-release
			return tx.Bucket([]byte(testBucket)).Put([]byte("consume"), make([]byte, 1<<20))
		})
	}()
	<-started
	result := make(chan db.CompactResult, 1)
	errs := make(chan error, 1)
	go func() {
		r, err := d.Compact(t.Context(), db.CompactOptions{MinReclaimBytes: stats.Reclaimable})
		result <- r
		errs <- err
	}()
	// The writer commits after the first measurement, while the gate drains.
	paused := false
	deadline := time.Now().Add(5 * time.Second)
	for time.Now().Before(deadline) {
		d.gate.mu.Lock()
		paused = d.gate.resume != nil
		d.gate.mu.Unlock()
		if paused {
			break
		}
	...
	require.Equal(t, "reclaimable space below threshold", r.Reason)
	require.Less(t, r.Reclaimable, stats.Reclaimable)
}
