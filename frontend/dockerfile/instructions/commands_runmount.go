...
func parseMount(val string, expander SingleWordExpander) (*Mount, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to parse csv mounts"+": %w", err)
	}
	...
	for _, field := range fields {
		...
		if !ok {
			...
			case "required":
				if m.Type == MountTypeSecret || m.Type == MountTypeSSH {
					...
					return nil, fmt.Errorf("unexpected key '%s' for mount type '%s'", key, m.Type)
				}
			default:
		...
		if expander != nil {
			...
			if idx := strings.IndexByte(value, '$'); idx != -1 && idx != len(value)-1 {
				return nil, fmt.Errorf("'%s' doesn't support variable expansion, define alias stage instead", key)
			}
		} else {
		...
		case "type":
			...
			if _, ok := allowedMountTypes[v]; !ok {
				return nil, suggest.WrapError(fmt.Errorf("unsupported mount type %q", value), value, allMountTypes(), true)
			}
			m.Type = v
		case "from":
			m.From = value
		...
		case "sharing":
			...
			if _, ok := allowedSharingModes[v]; !ok {
				return nil, suggest.WrapError(fmt.Errorf("unsupported sharing value %q", value), value, allShareModes(), true)
			}
			m.CacheSharing = v
		case "mode":
			mode, err := strconv.ParseUint(value, 8, 32)
			if err != nil {
				return nil, fmt.Errorf("invalid value %s for mode", value)
			}
			m.Mode = &mode
		case "uid":
			uid, err := strconv.ParseUint(value, 10, 32)
			if err != nil {
				return nil, fmt.Errorf("invalid value %s for uid", value)
			}
			m.UID = &uid
		case "gid":
			gid, err := strconv.ParseUint(value, 10, 32)
			if err != nil {
				return nil, fmt.Errorf("invalid value %s for gid", value)
			}
			m.GID = &gid
		case "env":
			m.Env = &value
	...
	if !fileInfoAllowed {
		if m.Mode != nil {
			return nil, fmt.Errorf("mode not allowed for %q type mounts", m.Type)
		}
		if m.UID != nil {
			return nil, fmt.Errorf("uid not allowed for %q type mounts", m.Type)
		}
		if m.GID != nil {
			return nil, fmt.Errorf("gid not allowed for %q type mounts", m.Type)
		}
	}
	...
	if m.CacheSharing != "" && m.Type != MountTypeCache {
		return nil, fmt.Errorf("invalid cache sharing set for %v mount", m.Type)
	}
