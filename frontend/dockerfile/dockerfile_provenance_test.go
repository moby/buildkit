		...
		for _, mode := range []string{"", "min", "max"} {
			...
			t.Run(strings.Join(tname, "-"), func(t *testing.T) {
				...
				if slsaVersion == "v1" {
					...
					require.Equal(t, "https://mobyproject.org/buildkit@v1", pred.BuildDefinition.BuildType)
					require.Equal(t, "", pred.RunDetails.Builder.ID)

					require.Equal(t, "", pred.BuildDefinition.ExternalParameters.ConfigSource.URI)

					args := pred.BuildDefinition.ExternalParameters.Parameters.Args
					if isClient {
						require.Equal(t, "", pred.BuildDefinition.ExternalParameters.Parameters.Frontend)
						require.Equal(t, 0, len(args), "%v", args)
						require.False(t, pred.RunDetails.Metadata.Completeness.Parameters)
						require.Equal(t, "", pred.BuildDefinition.ExternalParameters.ConfigSource.Path)
					} else if isGateway {
						require.Equal(t, "gateway.v0", pred.BuildDefinition.ExternalParameters.Parameters.Frontend)

						if mode == "max" || mode == "" {
							require.Equal(t, 4, len(args), "%v", args)
							require.True(t, pred.RunDetails.Metadata.Completeness.Parameters)
						...
						require.Equal(t, "dockerfile.v0", pred.BuildDefinition.ExternalParameters.Parameters.Frontend)
					...
					require.Equal(t, 2, len(pred.BuildDefinition.ExternalParameters.Parameters.Locals), "%+v", pred.BuildDefinition.ExternalParameters.Parameters.Locals)
					require.Equal(t, "context", pred.BuildDefinition.ExternalParameters.Parameters.Locals[0].Name)
					require.Equal(t, "dockerfile", pred.BuildDefinition.ExternalParameters.Parameters.Locals[1].Name)
					...
					require.True(t, pred.RunDetails.Metadata.Completeness.Environment)
					require.Equal(t, platforms.Format(platforms.Normalize(platforms.DefaultSpec())), pred.BuildDefinition.InternalParameters.Environment.Platform)

					require.False(t, pred.RunDetails.Metadata.Completeness.Materials)
					...
					if mode == "max" || mode == "" {
						...
						require.NotNil(t, pred.BuildDefinition.ExternalParameters.BuildConfig)

						require.Equal(t, 3, len(pred.BuildDefinition.ExternalParameters.BuildConfig.Definition))
					} else {
						...
						require.Nil(t, pred.BuildDefinition.ExternalParameters.BuildConfig)
					}
				} else {
					type stmtT struct {
...
`)
			...
			if slsaVersion == "v1" {
				...
				if isClient {
					require.Empty(t, pred.BuildDefinition.ExternalParameters.Parameters.Frontend)
					require.Equal(t, "", pred.BuildDefinition.ExternalParameters.ConfigSource.URI)
					require.Equal(t, "", pred.BuildDefinition.ExternalParameters.ConfigSource.Path)
				} else {
					require.NotEmpty(t, pred.BuildDefinition.ExternalParameters.Parameters.Frontend)
				...
				require.Equal(t, 0, len(pred.BuildDefinition.ExternalParameters.Parameters.Locals))

				require.True(t, pred.RunDetails.Metadata.Completeness.Materials)
				require.True(t, pred.RunDetails.Metadata.Completeness.Environment)
