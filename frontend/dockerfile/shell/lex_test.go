...
func TestShellParser4Words(t *testing.T) {
	...
	for _, mode := range []int{modeNormal, modeOnlySetEnv} {
		...
		for scanner.Scan() {
			...
			words := strings.Split(line, "|")
			require.Lenf(t, words, 2, "Error in '%s'(line %d) - should be exactly one | in: %q", fn, lineNum, line)
			...
			require.Lenf(t, result, len(expected), "Error on line %d. %q was suppose to result in %q, but got %q instead", lineNum, test, expected, result)
