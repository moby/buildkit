...
`
	...
	require.True(t, *cfg.Workers.OCI.Enabled)
	require.Equal(t, "overlay", cfg.Workers.OCI.Snapshotter)
	require.True(t, cfg.Workers.OCI.Rootless)
	require.False(t, *cfg.Workers.OCI.GC)
	...
	require.Len(t, cfg.Workers.Containerd.Platforms, 1)
	require.Equal(t, "containerd.sock", cfg.Workers.Containerd.Address)

	require.Empty(t, cfg.Workers.OCI.GCPolicy)
	...
	require.Len(t, cfg.Workers.Containerd.GCPolicy, 4)

	require.Nil(t, cfg.Workers.Containerd.GC)
	require.True(t, cfg.Workers.Containerd.GCPolicy[0].All)
	require.Len(t, cfg.Workers.Containerd.GCPolicy[0].Filters, 1)
	...
	require.False(t, cfg.Workers.Containerd.GCPolicy[1].All)
	...
	require.Empty(t, cfg.Workers.Containerd.GCPolicy[1].Filters)

	require.False(t, cfg.Workers.Containerd.GCPolicy[2].All)
	...
	require.False(t, cfg.Workers.Containerd.GCPolicy[3].All)
	...
	require.True(t, *cfg.Registries["docker.io"].PlainHTTP)
	require.True(t, *cfg.Registries["docker.io"].Insecure)
