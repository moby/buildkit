...
func applyOCIFlags(c *cli.Context, cfg *config.Config) error {
	...
	if c.IsSet("oci-worker-gc-keepstorage") {
		cfg.Workers.OCI.GCKeepStorage = c.Int64("oci-worker-gc-keepstorage") * 1e6
	}

	if c.IsSet("oci-worker-net") {
		cfg.Workers.OCI.NetworkConfig.Mode = c.String("oci-worker-net")
	}
	if c.IsSet("oci-cni-config-path") {
		cfg.Workers.OCI.NetworkConfig.CNIConfigPath = c.String("oci-cni-worker-path")
	}
	if c.IsSet("oci-cni-binary-dir") {
		cfg.Workers.OCI.NetworkConfig.CNIBinaryPath = c.String("oci-cni-binary-dir")
	}
	if c.IsSet("oci-cni-pool-size") {
		cfg.Workers.OCI.NetworkConfig.CNIPoolSize = c.Int("oci-cni-pool-size")
	}
