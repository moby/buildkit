...
func parseOutputCSV(s string) (client.ExportEntry, error) {
	...
	for _, field := range fields {
		...
		if !ok {
			return ex, fmt.Errorf("invalid value %s", field)
		}
	...
	if v, ok := ex.Attrs["output"]; ok {
		return ex, fmt.Errorf("output=%s not supported for --output, you meant dest=%s?", v, v)
	}
	ex.Output, ex.OutputDir, err = resolveExporterDest(ex.Type, ex.Attrs["dest"], ex.Attrs)
	if err != nil {
		return ex, fmt.Errorf("invalid output option: output"+": %w", err)
	}
	...
	if supportDir {
		...
		if dest != "" && dest != "-" {
			...
			if err != nil && !errors.Is(err, os.ErrNotExist) {
				return nil, "", fmt.Errorf("invalid destination file: %s: %w", dest, err)
			}
			if err == nil && fi.IsDir() {
		...
		if _, err := console.ConsoleFromFile(os.Stdout); err == nil {
			return nil, "", fmt.Errorf("output file is required for %s exporter. refusing to write to console", exporter)
		}
		return wrapWriter(os.Stdout), "", nil
	}
	// e.g. client.ExporterImage
	if dest != "" {
		return nil, "", fmt.Errorf("output %s is not supported by %s exporter", dest, exporter)
	}
	return nil, "", nil
}
