...
It builds a simple layer, exports it as OCI with Zstd compression, and confirms
the manifest uses the correct media type.
*/
func testBuildExportZstd(t *testing.T, sb integration.Sandbox) {
	/*
		Skipped on Windows for two reasons:
		1. AddMount() fails because Windows snapshots only support a single mount per layer,
		   so no additional mounts can be attached to a Run step. This could be worked around
		   by rewriting the LLB to avoid extra mounts (e.g., writing files directly without
		   mounting into the Run step).
		2. The OCI exporter fails because windowsLcowDiff (the Windows-specific component
		   responsible for computing layer differences) does not implement the Compare method
		   required to generate compressed layer diffs for export. This cannot be worked
		   around from the test side — it requires adding Compare support to windowsLcowDiff.
	*/
