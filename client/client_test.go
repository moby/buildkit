...
func testRawSocketMount(t *testing.T, sb integration.Sandbox) {
	...
	st := llb.Image("alpine:latest").
		...
		Run(llb.Shlex(`curl --unix-socket /tmp/test.sock http://./'`),
	...
	// Now the same thing but directly with the raw provider instead of going through the SSH provider
	called.Store(false) // reset the called flag
	raw, err = sshprovider.NewRawProvider([]sshprovider.RawConfig{{
