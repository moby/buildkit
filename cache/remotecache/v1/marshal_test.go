...
// Remotes that reference the same blobs through different providers must not be
// deduplicated either: if the provider of the first one cannot resolve them, the
// second is the only candidate left to fall back to.
func TestAddResultKeepsSameBlobsFromDifferentProviders(t *testing.T) {
	ts := time.Now()
	it := &item{dgst: dgst("rec")}

	it.addResult(testResult(ts, unresolvableProvider{}, "main"))
	it.addResult(testResult(ts, resolvableProvider{}, "main"))
	it.addResult(testResult(ts, resolvableProvider{}, "main"))
...
// Same fallback, but for two remotes that differ only in their provider, and
// reaching the record through two separate Add calls, as two exporters
// converging on the same cache key do.
func TestMarshalFallsBackToUsableProvider(t *testing.T) {
	ts := time.Now()

	cc := NewCacheChains()
	_, _, err := cc.Add(dgst("rec"), nil, []solver.CacheExportResult{
		testResult(ts, unresolvableProvider{}, "main"),
	})
	require.NoError(t, err)
	_, _, err = cc.Add(dgst("rec"), nil, []solver.CacheExportResult{
		testResult(ts, resolvableProvider{}, "main"),
