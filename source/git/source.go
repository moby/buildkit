...
func (gs *gitSourceHandler) shaToCacheKey(sha, ref string) string {
	...
	if gs.src.Checksum != "" {
		// checksum is covered by sha, but generate a key with the explicit checksum
		// so as to enforce verification
		key += "?checksum=" + gs.src.Checksum
	}
	if gs.src.Subdir != "" {
		key += ":" + gs.src.Subdir
	}
...
func (gs *gitSourceHandler) CacheKey(ctx context.Context, g session.Group, index int) (string, string, solver.CacheOpts, bool, error) {
	...
	if refCommitFullHash != "" {
		...
		// gs.src.Checksum is verified after checking out the commit
