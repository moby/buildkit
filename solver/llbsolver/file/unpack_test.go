...
func TestUnpackDuplicateDirectoryTimes(t *testing.T) {
	for _, tc := range []struct {
		name  string
		times [2]int64
	}{
		{name: "newer last", times: [2]int64{100, 200}},
		{name: "older last", times: [2]int64{200, 100}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			dest := t.TempDir()
			buf := bytes.NewBuffer(nil)
			...
			for _, timestamp := range tc.times {
				for _, name := range []string{"dir/", "dir/child/"} {
					require.NoError(t, tw.WriteHeader(&tar.Header{
						Name:     name,
			...
			writeTarFile(t, tw, "dir/child/file", "content")
			require.NoError(t, tw.Close())
			...
			for _, name := range []string{"dir", "dir/child"} {
				...
				require.NoError(t, err)
				require.Equal(t, tc.times[1], fi.ModTime().Unix(), "last directory header must determine mtime for %s", name)
			}
		})
	}
}
