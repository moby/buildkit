	...
	for _, action := range f.op.Actions {
		...
		case *pb.FileAction_Copy:
			...
			if action.SecondaryInput != -1 && int(action.SecondaryInput) < f.numInputs {
				addSelector(selectors, int(action.SecondaryInput), p.Src, p.AllowWildcard, p.FollowSymlink, p.IncludePatterns, p.ExcludePatterns, p.RequiredIncludePatterns)
...
func addSelector(m map[int][]opsutils.Selector, idx int, sel string, wildcard, followLinks bool, includePatterns, excludePatterns, requiredIncludePatterns []string) {
	s := opsutils.Selector{
		Path:                    sel,
		...
		RequiredIncludePatterns: requiredIncludePatterns,
	}
